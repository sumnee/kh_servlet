...
public class MemberDao {
	...
	//DAO에서 쿼리문 오타 잘 확인하기! 결과값 execute로 잘 받기
	...
	public int insertMember(Connection conn, Member member) {	
		...
		try {
			//위치홀더에 값을 넣을 때 순서 중요! SQL 테이블과 맞추기
			PreparedStatement pstmt = conn.prepareStatement(query);
	...
	public int updateMember(Connection conn, Member member) {
		int result = 0;
		String sql = "UPDATE MEMBER_TBL SET MEMBER_PW = ?, MEMBER_EMAIL = ?, MEMBER_ADDR = ?, MEMBER_PHONE = ?, MEMBER_HOBBY = ? WHERE MEMBER_ID = ?";
		try {
